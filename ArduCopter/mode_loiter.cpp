...
bool ModeLoiter::init(bool ignore_checks) {
  if (!copter.failsafe.radio) {
    ...
    // clear out pilot desired acceleration in case radio failsafe event occurs
    // and we do not switch to RTL for some reason
  ...
  pos_control->set_max_speed_accel_z(-get_pilot_speed_dn(), g.pilot_speed_up,
                                     g.pilot_accel_z);
...
void ModeLoiter::precision_loiter_xy() {
  ...
  copter.precland.get_target_velocity_cms(inertial_nav.get_velocity_xy_cms(),
                                          target_vel);
...
void ModeLoiter::run() {
  ...
  if (!copter.failsafe.radio) {
    ...
    target_climb_rate = constrain_float(
        target_climb_rate, -get_pilot_speed_dn(), g.pilot_speed_up);
  } else {
    // clear out pilot desired acceleration in case radio failsafe event occurs
    // and we do not switch to RTL for some reason
    loiter_nav->clear_pilot_desired_acceleration();
  ...
  case AltHoldModeState::MotorStopped:
    ...
    pos_control->relax_z_controller(
        0.0f); // forces throttle output to decay to zero
    loiter_nav->init_target();
  ...
  case AltHoldModeState::Flying:
    ...
    if (precision_loiter_old_state && !_precision_loiter_active) {
      // prec loiter was active, not any more, let's init again as user takes
      // control
      loiter_nav->init_target();
    }
