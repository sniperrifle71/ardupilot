...
public:
  ...
  enum SerialProtocol {
    ...
    SerialProtocol_MAVLink2 =
        2, // do not use - use MAVLink and provide instance of 1
    ...
    SerialProtocol_FrSky_SPort_Passthrough =
        10, // FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
    SerialProtocol_Lidar360 =
        11, // Lightware SF40C, TeraRanger Tower or RPLidarA2
    SerialProtocol_Aerotenna_USD1 =
        12, // USD1 support - deprecated, users should use Rangefinder
    ...
    SerialProtocol_IBUS_Telem =
        49, // i-BUS telemetry data, ie via sensor port of FS-iA6B
    SerialProtocol_OpenMV = 50, // OpenMV serial protocol
    SerialProtocol_NumProtocols // must be the last value
  };

  // get singleton instance
  static AP_SerialManager *get_singleton(void) { return _singleton; }
  ...
  // find_serial - searches available serial ports that allows the given
  // protocol
  //  instance should be zero if searching for the first instance, 1 for the
  //  second, etc returns uart on success, nullptr if a serial port cannot be
  //  found
  // note that the SERIALn_OPTIONS are applied if the port is found
  AP_HAL::UARTDriver *find_serial(enum SerialProtocol protocol,
                                  uint8_t instance) const;

  // have_serial - return true if we have the corresponding serial protocol
  // configured
  bool have_serial(enum SerialProtocol protocol, uint8_t instance) const;

  // find_baudrate - searches available serial ports for the first instance that
  // allows the given protocol
  //  instance should be zero if searching for the first instance, 1 for the
  //  second, etc returns the baudrate of that protocol on success, 0 if a
  //  serial port cannot be found
  uint32_t find_baudrate(enum SerialProtocol protocol, uint8_t instance) const;

  // find_portnum - find port number (SERIALn index) for a protocol and
  // instance, -1 for not found
  int8_t find_portnum(enum SerialProtocol protocol, uint8_t instance) const;

  // get the passthru ports if enabled
  bool get_passthru(AP_HAL::UARTDriver *&port1, AP_HAL::UARTDriver *&port2,
                    uint8_t &timeout_s, uint32_t &baud1, uint32_t &baud2);
  ...
  void set_protocol_and_baud(uint8_t sernum, enum SerialProtocol protocol,
                             uint32_t baudrate);
  ...
  public:
    ...
    uint32_t baudrate() const { return AP_SerialManager::map_baudrate(baud); }
...
private:
  ...
  bool protocol_match(enum SerialProtocol protocol1,
                      enum SerialProtocol protocol2) const;
